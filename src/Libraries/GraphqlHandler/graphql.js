(function () {
  ...
  if (typeof XMLHttpRequest !== "undefined" && typeof fetch !== "undefined") {
    __doRequest = function (
      ...
      headersRequest.append("Content-Type", contentType);
      headersRequest.append("Accept", accept);
      for (var key in headers) {
        headersRequest.append(key, headers[key]);
      }
      fetch(url, { method: method, body: body, headers: headersRequest })
        .then((response) => {
          if (!response.ok) callback(response, response.status);
          return response.json();
        })
        .then((r) => callback(r, 200))
        .catch((err) => callback(err, err.status));
    };
  ...
  function __isTagCall(strings) {
    return (
      Object.prototype.toString.call(strings) === "[object Array]" &&
      strings.raw
    );
  }
    ...
    (query.match(fragmentRegexp) || []).forEach(function (fragment) {
      ...
      if (fragment) {
        ...
        if (
          ...
          that
            .collectFragments(fragment, fragments)
  ...
  GraphQLClient.prototype.autoDeclare = function (query, variables) {
    ...
    return query.replace(GraphQLClient.AUTODECLARE_PATTERN, function () {
      ...
      for (var key in variables) {
        ...
        var mappedType =
          typeof mapping === "function" ? mapping(value) : mapping;
  ...
  GraphQLClient.prototype.buildQuery = function (query, variables) {
    return this.autoDeclare(
      this.processQuery(query, this._fragments),
      variables
    );
  };
      ...
      var subQuery = that._transaction[mergeName][method]
        .map(function (merge) {
          ...
          var query = merge.query.replace(/\$([^\.\,\s\)]*)/g, function (_, m) {
            ...
            var matchingKey = Object.keys(merge.variables).filter(function (
              key
            ) {
  ...
  GraphQLClient.prototype.createHelpers = function (sender) {
    ...
    this.run = function (query) {
      return sender(query, _originalQuery, this.type, {});
    };
  };
  ...
  GraphQLClient.prototype.fragment = function (fragment) {
    if (typeof fragment == "string") {
      var _fragment =
        this._fragments[fragment.replace(/\./g, FRAGMENT_SEPERATOR)];
      if (!_fragment) {
